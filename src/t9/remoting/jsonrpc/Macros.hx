...
class Macros
{
	...
	static function buildRemoteProxyClassInternal(className: String, connectionExpr :Expr, ?implementExpr :Expr) :Expr
	{
		...
		var superClassTypePath :TypePath = {params:[], pack:["t9", "remoting", "jsonrpc"], name:"RPCProxy"};
	...
	#if macro
	static var RPC_CONNECTION_TYPE : ComplexType = ComplexType.TPath({sub:null, params:[], pack:["t9", "remoting", "jsonrpc"], name:"ConnectionClient"});
	#end
}
