...
class RPCTest
{
	static var WEBSOCKET_PORT :Int = 8005;
	static var WEBSOCKET_SERVER :WebSocketServer;

	static function createWebSocketServer(cb :Void->Void) :Void
	{
		...
		WEBSOCKET_SERVER = new WebSocketServer(serverConfig);

		server.listen(WEBSOCKET_PORT, 'localhost', function() {
			Log.info(Date.now() + ' Websocket server running at http://localhost:' + WEBSOCKET_PORT);
			cb();
		});
	}
	...
	public function setup(cb :Void->Void)
	{
		createWebSocketServer(cb);
	}

	public function tearDown(cb :Err->Void)
	{
		if (WEBSOCKET_SERVER != null) {
			Log.info("WebSocketServer shutting down");
	...
	public function testBasicWebSocketConnection (onTestFinish :Err->Void, assert :Bool->?Dynamic->Void) :Void
	{
		...
		var step = new Step();
		var address = "http://localhost:" + WEBSOCKET_PORT;
		...
		var websocketServer :WebSocketServer = WEBSOCKET_SERVER;
	...
	// @AsyncTest
	// public function testRPC (onTestFinish :Err->Void, assert :Bool->?Dynamic->Void) :Void
	// {
	// }
}